...
public class ProgressBar extends PavWidget {
    GlyphLayout layout;
    TextureRegion hoverTexture = new TextureRegion();
    float displayedProgress = 0f;
    TextureRegion progressBG, progressFG;

    public ProgressBar(String text, BitmapFont font, ClickBehavior clickBehavior, TextureRegion progressFG, TextureRegion progressBG) {
        this.text = text;
        this.font = font;
...
//        sb.draw(progressBG,textX + layout.width, textY-32 + 4,192,32);
//        sb.draw(progressFG,textX + layout.width, textY-32 + 4,levelManager.getLevelProgress()*192/100,32);
        float actualProgress = MathUtils.clamp(levelManager.getLevelProgress(), 0f, 1f);
        displayedProgress = MathUtils.clamp(
            MathUtils.lerp(displayedProgress, actualProgress, 5f * Gdx.graphics.getDeltaTime()),
            0f, 1f
        );
        float totalWidth = 192f;


        sb.draw(progressBG, box.x + 22, box.y - 32, totalWidth, 32);

        sb.draw(progressFG, box.x + 22, box.y - 32, displayedProgress * totalWidth, 32);
    }
